...
def compute_band_powers(eegdata, fs):
    ...
    # Apply Hamming window: taper the edges of the signal to avoid frequency distortions
    ...
    feature_matrix = np.vstack((meanDelta, meanTheta, meanAlpha, meanBeta)).T

    feature_matrix = np.log10(feature_matrix)
...
def update_buffer_4channels(data_buffer, new_data, notch=False, filter_state=None):
    """
    Concatenates "new_data" into "data_buffer", and returns an array with
    the same size as "data_buffer". Specifically handles four channels of data.

    Args:
        data_buffer (numpy.ndarray): The buffer storing past data.
        new_data (numpy.ndarray): The new incoming data to add to the buffer.
        notch (bool): Whether to apply a notch filter to the new data.
        filter_state (numpy.ndarray or None): The initial state for the notch filter.

    Returns:
        tuple: A tuple containing the updated data buffer and filter state.
    """
    num_channels = 4  # Explicitly handling four channels

    # Reshape new_data if it's one-dimensional, assuming it should be a single channel
    if new_data.ndim == 1:
        new_data = new_data.reshape(-1, 1)
    
    # Make sure new_data has the right number of channels
    if new_data.shape[1] != num_channels:
        raise ValueError(f"new_data must have {num_channels} channels, found {new_data.shape[1]}")

    if notch:
        # Initialize filter_state if None, ensuring it matches the expected shape for 4 channels
        if filter_state is None:
            filter_state = np.tile(lfilter_zi(NOTCH_B, NOTCH_A), (num_channels, 1)).T
        
        # Apply notch filter
        new_filtered_data = np.zeros_like(new_data)
        for i in range(num_channels):
            new_filtered_data[:, i], filter_state[:, i] = lfilter(NOTCH_B, NOTCH_A, new_data[:, i], zi=filter_state[:, i])

        new_data = new_filtered_data

    # Update buffer
    new_buffer = np.concatenate((data_buffer, new_data), axis=0)
    new_buffer = new_buffer[-data_buffer.shape[0]:, :]  # Keep buffer size constant
